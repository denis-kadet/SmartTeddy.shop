...
$('#billing_first_name, #billing_email, #billing_address_1, #billing_city, #billing_postcode, #billing_state').attr('required', 'required');
...
//header active
    ...
    $('.support__contact-arrows').on("click", function () {
        $('.support__section').toggleClass('support__section-arrows');
        $('.support__contact-arrows').toggleClass('support__contact-arrows_down');
        $('#wpcf7-f27-o1').toggle();
    })


    $('.number-plus, .number-minus').on("click", function () {
        $('.button__cart').trigger('click');
    })

    $(document).on('keyup mouseup keydown change blur', 'input', function () {
        $('#quantity_61a47633463ed').html($(this).val());
    });

    $(".checkbox-btn").on("click", function () {
        $('.woocommerce-Price-amount').addClass('woo__price-amount');
    });
    ...
    $('#billing_phone').inputmask("+1(999)999-9999");
    $('input[name="billing_phone"]').inputmask("+1(999)999-9999");

    // jQuery.validator.addMethod("checkMaskPhone", function(value, element) {
    //     return /\+\d{1}\(\d{3}\)\d{3}-\d{4}/g.test(value);
    // });
    jQuery.validator.addMethod("checkMaskMail", function(value, element) {
    return /^[a-z0-9_-]+@[a-z0-9-]+\.[a-z]{2,6}$/i.test(value);
    });
    $('#checkout__id').each(function() {
        $(this).validate({
            errorClass: "error fail-alert has-error",
            ...
            showErrors: function(errorMap, errorList) {
                let name = Object.getOwnPropertyNames(errorMap);
                jQuery(this.currentForm).find('input[name="' + name + '"]').parent().addClass('has-error');
                this.defaultShowErrors();
            },
            // errorPlacement: function(error, element) {
            //     if( jQuery(element).attr('id') == 'fPhone' ) {
            //         console.log('fnum');
            //         error.insertBefore('#errorMarker');
            //     } else {
            ...
            messages: {
                billing_email: {
                    ...
                    checkMaskMail: "Your email address must be in the format name@domain.com"
                },
            }
...
function windowUp() {
    document.getElementsByClassName('newsletter__form-wrap')[0].style.display = "block";
    document.body.style.backgroundColor = 'rgba(0,0,0,0.5)';

}

if (sessionStorage.getItem('key') == null) {
    setTimeout('windowUp();', 15000);
}

function windowClose() {
    document.getElementsByClassName('newsletter__form-wrap')[0].style.display = "none";
